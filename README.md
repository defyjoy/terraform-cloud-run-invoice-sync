...
subscribe) and an email address (`notification_email` in `.auto.tfvars`). Terraform creates the
email channel but can't verify it (Google requires a code sent to the actual inbox) — see
`docs/bootstrap-runbook.md` for that one-time step and for forcing a test failure.
